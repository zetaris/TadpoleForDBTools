...
public class MongoDBTableColumn {
	 ...
	 * @return
	 */
	public static List<CollectionFieldDAO> tableColumnInfoFlat( DBObject dbObject) {
		Map<String, Boolean> mapIndex = new HashMap<String, Boolean>();
		...
		try {
			...
			for (String name : names) {
				...
				column.setSearchName(name);
				
				// 자식들이 있는 케이스 이면
				if( dbObject.get(name) instanceof BasicDBObject ) {
					makeTableColumnFlat(retColumns, column, (BasicDBObject)dbObject.get(name));
				}
	 ...
	 * @param dbObject
	 */
	private static void makeTableColumnFlat(List<CollectionFieldDAO> retColumns, CollectionFieldDAO column, BasicDBObject dbObject) {
		Set<String> names = dbObject.keySet();
		...
		for (String name : names) {	
			...
			if( dbObject.get(name) instanceof BasicDBObject ) {
				makeTableColumnFlat(retColumns, columnSub, (BasicDBObject)dbObject.get(name));
			}
			
			listChildField.add(columnSub);		
		}
		
		retColumns.addAll(listChildField);
	}
